...
for page in range(1, pages):
    ...
    #     lambda x: x.has_key('href') and x.attrs['href'].startswith('http://www.vgchartz.com/game/'),
    #     # discard the first 10 elements because those
    #     # links are in the navigation bar
    #     soup.find_all("a")
    # ))[10:]
    game_tags = list(soup.find_all("a", href=lambda x: x and 'www.vgchartz.com/game' in x))[14:]
    # print(game_tags)
    ...
    for tag in game_tags:
        ...
        # print(tag.parent.parent.find_all("td")[0].text)
...
# }
s1=pd.Series(rank, name='Rank')
s2=pd.Series(gname, name='Name')
s3=pd.Series(platform, name='Platform')
s4=pd.Series(year, name='Year')
s5=pd.Series(genre, name='Genre')
s6=pd.Series(critic_score, name='Critic_Score')
s7=pd.Series(user_score, name='User_Score')
s8=pd.Series(publisher, name='Publisher')
s9=pd.Series(developer, name='Developer')
s10=pd.Series(sales_na, name='NA_Sales')
s11=pd.Series(sales_pal, name='PAL_Sales')
s12=pd.Series(sales_jp, name='JP_Sales')
s13=pd.Series(sales_ot, name='Other_Sales')
s14=pd.Series(sales_gl, name='Global_Sales')

df = pd.concat([s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14], axis=1)
print(df.columns)
df = df[[
